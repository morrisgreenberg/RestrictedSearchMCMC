        ...
        foreach(datatype=data_model, .combine='rbind') %:%
          foreach(i=seeds, .combine='rbind') %:%
            foreach(k=replicates, .combine='rbind') %dopar% {
              ...
              auc_orig <- pROC::auc(roc(trueDAG_skel_lower,
                                        mean_edge_est[lower.tri(mean_edge_est)]))
              auc_weighted <- pROC::auc(roc(trueDAG_skel_lower,
              ...
              #                             weighted.mean, w=results_2$weight[burn_in:Bs]))
              # auc_orig2 <- pROC::auc(roc(trueDAG_skel_lower,
              #                           mean_edge_est2[lower.tri(mean_edge_est2)]))
              # auc_weighted2 <- pROC::auc(roc(trueDAG_skel_lower,
